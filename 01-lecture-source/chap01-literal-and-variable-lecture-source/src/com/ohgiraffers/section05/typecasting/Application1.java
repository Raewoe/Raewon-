...
public class Application1 {
    ...
    public static void main(String[] args){
        ...
        *  1-3. 문자형(ch)은 int형으로 자동형변환이 된다.
        *  1-4. 논린형은 형변환 규칙에서 제외된다.
        * 2. 강제형변환(명시적 형변환) : 형변환(cast) 연산자를 이용한 강제적으로 수행하는 형변환
        *  2-1. 자동형변환이 되지 않는 경우 강제형변환이 필요하다.
        *  */

        /* 자동형변환 규칙테스트 */
        /* 1-1-1. 작은 자료형에서 큰 자료형으로 자동형변환된다.*/
        ...
        //int result = num1 + num2;     //자동으로 큰 자료형인 long타입으로 변경 후에 계산하기 때문에 int형 변수에 값을 담을 수 있다.

        long result = num1 + num2;      //int + long은 다른 자료형이라 데이터 손실이 발생하지 않는 int + long 변경을 자동을 수행 후 연산한다.
        ...
        /* 정수를 실수로 변경할 때 소수점 자리수가 없어도 실수형태로 표현이 가능하다. 이 때 데이터 손실이 없기 때문에 자동형변환이 가능하다.*/
