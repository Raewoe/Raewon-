...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * 특히 메소드 재정의(overiding)라는 기술을 이용해서 부모가 가진 메소드를 재정의하는 것도 가능하다.
        * */

        /*
        * 메소드 재정의(overiding)이란?
        * 부모가 가지는 메소드 선언부를 그대로 사용하면서 자식 클래스가 정의한 메소드대로
        * 동작하도록 구현 몸체 부분을 새롭게 다시 작성하는 기술이다.
        ...
        *      적용된다(유지 보수성 증가)
        * 2. 클래스간의 계층 관계가 형성되며 다향성의 문법적인 토대가 된다.
        * */

        /*
        * 상속으로 인한 단점
        * 1. 부모클래스의 기능을 추가, 변경시에 자식클래스가 정상 동작하는지 예측이 힘들다.
        *    상속 구조가 복잡해질 수록 그 영향에 대한 예측이 힘들어 이런 단점이 유지보수성이
        *    증가한다는 장점과는 반대로 유지보수에 악영향을 미친다.
        * 2. 또한 부모클래스 변경 또한 쉽지 않다. 자식클래스에서 중요하게 사용하는 기능인 경우 부모클래스 변겸할 시
        *    자식 클래스에 모두 영향을 줄 수 있다. 역시 유지보수에 악영향을 미친다.
        * 3. 부모클래스에서는 의미있었던 기능들이 자식 클래스에서는 무의미할 수 있다.(불필요한 기능이 추가됨)
        * */

        /*
        * 장정과 단점을 고려했을 때 상속은 재사용이라는 장점만을 바라보게 되면 요용의 가능성이 있기 때문에 유지보수에
        * 좋지 않은 코드를 잘성할 확률이 높다.
        * 상속은 IS-A 관계로 구분되는 경우에만 사용해야 된다.
        * */
        ...
        * */
        ...
        /*
        * 적절한 책임을 수행하는 객체 또한 그 객체만 수행할 수 있는 기능이라기 보다 역학의 관점으로 바라봐야한다.
        * 역할이란 동인한 동작을 수행하는 것을 정의한 것이며, 대체 가능성을 의미한다.
