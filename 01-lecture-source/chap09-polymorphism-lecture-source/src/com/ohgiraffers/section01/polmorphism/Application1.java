...
public class Application1 {
    public static void main(String[] args) {
        ...
        * 그렇기 때문에 하나으ㅢ 타입으로 여러 타입의 인스턴스를 처리할 수 있기도 하고,
        * 하나의 메소드 호출로 객체별로 각기 다른 방법으로 동작하게 할 수 있다.
        * */

        /*
        * 다형성은 객체지향 프로그래밍의 3대 특징(캡슐화, 상속, 다형성) 중 하나이며,
        * 객체지향 프로그램 꽃이라고 불리우 정도로 활용성이 높고 장점이 많다.
        ...
        *    하나의 호출로 여러가지 동작을 수행할 수 있다는 측면에서 오버로딩을 다형성이라고 보기도 한다.
        ...
        * 런다입 당시의 실제 객체가 가진 오버라이딩된 메소드로
        * 바인딩이 바뀌어 동잘하는 것을 의민한다.
        * */

        a1.cry();
        a2.cry();

        //a1.jump();     //에러발생 , 부모 클래스에는 없고 자식 클래스에만 동작은 작동하지 않는다
        //a2.bite();     //에러발생 , 부모 클래스에는 없고 자식 클래스에만 동작은 작동하지 않는다

        /* 7. 타입 형변환 */

        System.out.println("============타임 형변환 확인==============");

        ((Rabbit)a1).jump();
        ((Tiger)a2).bite();

        //((Tiger)a1).bite();      //에러발생, 토끼는 호랑이가 될 수 없다.
        ...
        * */
        ...
        Animal animal2 = (Animal) new Rabbit();       //down-casting 명시적 형변환

        Rabbit rabbit1 = (Rabbit) animal1;            //down-casting 명시적 형변환
        //Rabbit rabbit2 = animal2;                   //down-casting 묵시적 형변환 안 됨
